...
def get_all_analyzable_deck_names(df):
    deck_names = set()
    for col in ['my_deck', 'opponent_deck']:
        if col in df.columns:
            valid_decks = df[col].astype(str).replace('', pd.NA).dropna()
            deck_names.update(d for d in valid_decks.tolist() if d and d.lower() != 'nan')
    return sorted(list(deck_names))

def get_all_types_for_archetype(df, deck_name):
    types = set()
    for col_deck, col_type in [('my_deck', 'my_deck_type'), ('opponent_deck', 'opponent_deck_type')]:
        if col_deck in df.columns and col_type in df.columns:
            matching_rows = df[df[col_deck] == deck_name]
            valid_types = matching_rows[col_type].astype(str).replace('', pd.NA).dropna()
            types.update(t for t in valid_types.tolist() if t and t.lower() != 'nan')
    if not types:
        return [ALL_TYPES_PLACEHOLDER]
    return [ALL_TYPES_PLACEHOLDER] + sorted(list(types))
...
def show_analysis_section(original_df):
    ...
    if selected_focus_deck and selected_focus_deck != SELECT_PLACEHOLDER:
        ...
        # display_general_deck_performance(df_for_analysis) のコードをここに
        st.subheader("全デッキアーキタイプ パフォーマンス概要")
        all_deck_archetypes = get_all_analyzable_deck_names(df_for_analysis) 
        ...
        for deck_a_name in all_deck_archetypes:
            ...
            games_as_my_deck_df = df_for_analysis[df_for_analysis['my_deck'] == deck_a_name]
            wins_as_my_deck = len(games_as_my_deck_df[games_as_my_deck_df['result'] == '勝ち'])
            count_as_my_deck = len(games_as_my_deck_df)
            games_as_opponent_deck_df = df_for_analysis[df_for_analysis['opponent_deck'] == deck_a_name]
            ...
            games_involving_deck_a = df_for_analysis[(df_for_analysis['my_deck'] == deck_a_name) | (df_for_analysis['opponent_deck'] == deck_a_name)]
